function [ params ] = holoLens2Params(params)
    ...
    %params.camera.rotation.wrt.marker = deg2rad([1.0 3.0 6.0]); % this is near optimal for query 1
    %params.camera.rotation.wrt.marker = deg2rad([-1.0 3.0 2.0]); % this is near optimal for query 2
    params.camera.rotation.wrt.marker = deg2rad([0.0 3.0 4.0]); % this is aims to be generic
    %params.camera.origin.wrt.marker = 0.023 * [3; 13; -5]; % this is near optimal for query 1
    %params.camera.origin.wrt.marker = 0.023 * [3; 18; -8]; % this is near optimal for query 2
    params.camera.origin.wrt.marker = 0.023 * [3; 15.5; -6.5]; % this aims to be generic
    params.camera.sensor.size = [756, 1344]; % height, width
    params.camera.fl = 1015; % in pixels
    %params.HoloLensViconSyncConstant = 28.9 * 1000; % [ms]; expected: <28.8, 29.0> % this is near optimal for query 1
    %params.HoloLensViconSyncConstant = 28.9 * 1000; % [ms]; expected: <28.8, 29.4> % this is near optimal for query 2
    params.HoloLensViconSyncConstant = 28.9 * 1000; % [ms]; expected: <28.8, 29.4> % this aims to be generic
