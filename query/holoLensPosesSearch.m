...
nTds = 15;
nOds = 15;
avgTerrors = repmat(-1,nTds,nOds);
avgRerrors = repmat(-1,nTds,nOds);
%p = parpool;
%opts = parforOptions(p,'RangePartitionMethod','fixed','SubrangeSize',2);
%parfor td=1:nTds
for td=0:nTds-1
for od=0:nOds-1
fprintf('%0.2f%% finished.\n', 100*(td*nTds+od)/(nTds*nOds));
...
pts = ptsWhitelisted(1:nWhitelistedPts-td,:);
pts_ref = ptsWhitelisted_ref(1:nWhitelistedPts-td,:);
...
fclose(readmeFile);
nQueriesWithoutEnd = nQueries-max([td, od]);
...
fprintf('HoloLensTranslationDelay: %d, HoloLensOrientationDelay; %d\n', td, od);
avgTerrors(td+1,od+1) = avgTerror;
avgRerrors(td+1,od+1) = avgRerror;
